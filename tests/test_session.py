...
class TestHasSessionCookieInHeaders:
    """has_session_cookie_in_headers関数のテスト."""
    ...
    def test_returns_true_when_cookie_exists(self, mock_st_context):
        """セッションCookieが存在する場合はTrueを返す."""
        from app.services.session import has_session_cookie_in_headers

        mock_st_context.get.return_value = f"{SESSION_COOKIE_NAME}=abc123"

        result = has_session_cookie_in_headers()

        assert result is True

    def test_returns_false_when_no_cookie(self, mock_st_context):
        """セッションCookieがない場合はFalseを返す."""
        from app.services.session import has_session_cookie_in_headers
    ...
    def test_returns_false_when_other_cookies_only(self, mock_st_context):
        """他のCookieのみの場合はFalseを返す."""
        from app.services.session import has_session_cookie_in_headers

        mock_st_context.get.return_value = "other=value; another=test"
