...
from app.services.cache import get_firestore_client
...
from app.services.session_keys import SESSION_ID
...
def get_session_cookie() -> str | None:
    """Cookieからsession_idを取得（HTTPヘッダー優先）.
...
def get_session_id(cookie_manager: CookieManager | None = None) -> str | None:
    """session_stateまたはCookieからsession_idを取得（優先度順）.

    Args:
        cookie_manager: CookieManagerインスタンス（省略時はHTTPヘッダーから取得）
...
def invalidate_session(
    ...
    """セッションを無効化（Firestore + Cookie削除）.

    Args:
        session_id: セッションID
        cookie_manager: CookieManagerインスタンス（省略時はCookie削除しない）
    """
